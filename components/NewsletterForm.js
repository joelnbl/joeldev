...
const NewsletterForm = ({ title = 'Suscríbete' }) => {
  ...
  const subscribe = async (e) => {
    ...
    setError(false)
    setSubscribed(true)
    setMessage('¡Exitosamente! 🎉 Ahora está suscrito.')
  }
      ...
      <form className="flex flex-col sm:flex-row" onSubmit={subscribe}>
        <div>
          ...
          <input
            ...
            id="email-input"
            name="email"
            placeholder={subscribed ? 'Estás suscrito ! 🎉 ' : 'Ingrese su correo electrónico'}
            ref={inputEl}
            required
            type="email"
            disabled={subscribed}
        ...
        <div className="flex w-full mt-2 rounded-md shadow-sm sm:mt-0 sm:ml-3">
          <button
            ...
            disabled={subscribed}
          >
            {subscribed ? '¡Gracias!' : 'Inscribirse'}
          </button>
        </div>
      </form>
      {error && (
